...
class Depth(object):
    ...
    ----------
    filename : str
        String or path like of path to depth file
    ...
    >>> depth = Depth.read('filename.dep',grid.shape)
    
    Write to Delft3D-FLOW input file
    >>> Depth.write(depth, 'filename.dep')"""
    ...
    def write(dep, filename, **kwargs):
        ...
        print("Writing .dep file with shape", dep.shape, "to", filename)
        dep.values[np.isnan(dep.values)] = -999.0 # blank values
